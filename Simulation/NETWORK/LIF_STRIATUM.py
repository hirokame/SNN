...
class LIF_striatum:
    ...
    def step(self, dt, I, method=0):
        ...
        self.theta += dw
        amp = self.res_amp*np.exp(-self.t/1500) # resonantの振幅は1000msで(1/e)倍になる設定
        #self.res = np.cos(self.theta)*amp # １乗
        #self.res = np.cos(self.theta)*abs(np.cos(self.theta))*amp ## ２乗
        #self.res = np.cos(self.theta)**3*amp #３乗
        self.res = np.cos(self.theta)**7*amp #７乗
        self.thrsres = self.thrs - self.res ## 実際のthreshold
